...
from experiments.scenarios import run_benign as run


def test_benign_convergence():
    cfg = RunConfig(n_honest=10, peer_set_size=7, num_rounds=50, global_seed=42)
    rows = run(cfg)
    last = rows[-1]
...
def test_determinism():
    cfg = RunConfig(global_seed=1234)
    a = run(cfg)
    b = run(cfg)
    assert len(a) == len(b)
...
def test_different_seed_differs():
    a = run(RunConfig(global_seed=1))
    b = run(RunConfig(global_seed=2))

    ...
    for r in range(0, 40, 3):
        for ex in range(0, 40, 5):
            for pw in (True, False):
                s = identity_score(r, 0, ex, pw, AGE_MAX, EXCHANGE_MAX, RELIABILITY_MAX)
                assert 0.0 <= s <= 1.0
...
def test_minimum_is_zero():
    assert identity_score(0, 0, 0, False, AGE_MAX, EXCHANGE_MAX, RELIABILITY_MAX) == 0.0


def test_maximum_is_one():
    assert identity_score(AGE_MAX, 0, EXCHANGE_MAX, True, AGE_MAX, EXCHANGE_MAX,
                          RELIABILITY_MAX) == 1.0
...
def test_score_falls_with_missed_heartbeats():
    ...
    half = identity_score(AGE_MAX, 0, EXCHANGE_MAX, True, AGE_MAX, EXCHANGE_MAX,
                          RELIABILITY_MAX, missed_total=RELIABILITY_MAX // 2)
    none = identity_score(AGE_MAX, 0, EXCHANGE_MAX, True, AGE_MAX, EXCHANGE_MAX,
                          RELIABILITY_MAX, missed_total=RELIABILITY_MAX)
...
def test_score_is_deterministic():
    args = (7, 0, 4, True, AGE_MAX, EXCHANGE_MAX, RELIABILITY_MAX)
    assert identity_score(*args) == identity_score(*args)


def test_age_and_exchange_combinations_are_monotone():
    base = identity_score(5, 0, 5, True, AGE_MAX, EXCHANGE_MAX, RELIABILITY_MAX)
    older = identity_score(10, 0, 5, True, AGE_MAX, EXCHANGE_MAX, RELIABILITY_MAX)
    busier = identity_score(5, 0, 10, True, AGE_MAX, EXCHANGE_MAX, RELIABILITY_MAX)
...
def test_reliability_in_range():
    for missed in range(0, 2 * RELIABILITY_MAX):
        assert 0.0 <= score_reliability(missed, RELIABILITY_MAX) <= 1.0

...
def _spec_fields(c, override=None):
    return dict(
        ...
        eclipse_targets=c["eclipse_targets"], sybil_rate=c["sybil_rate"], partition_groups=c["partition_groups"], poisoning=c["poisoning"], gossip_fanout=c["gossip_fanout"], refresh_period=c["refresh_period"], delay_rounds=c["delay_rounds"], churn_offline=c["churn_offline"],
        per_node_metrics=c["per_node_metrics"], trace_events=c["trace_events"],
    )


SWEEPABLE = ("sybil_rate", "flooding", "churn_period", "unresponsive_p", "selective_p", "delay_rounds", "churn_offline", "discovery_offers",

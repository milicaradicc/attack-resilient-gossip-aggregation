...
class ExperimentMetrics:
    ...
    def _malicious_share(self, node, scenario):
        if not node.peers:
            return 0.0
        malicious = scenario.byzantine_ids | scenario.sybil_ids
        return sum(1 for p in node.peers if p in malicious) / len(node.peers)
